...
class BridgeRepl(cmd.Cmd):
    ...
    def do_new_proof(self, line):
        try:
            args = line.split()
            if len(args) != 2:
                print(
                    f'Error: new_proof command exactly two arguments. Type "help" for help.'
                )
                return
            ...
            proof = None
            try:
                proof = XChainClaimProof(from_rpc_result=proof_dict)
            except Exception as e:
                print(f"Could not create proof from dict: {proof_dict} {e=}")
                return
            self.proof_aliases[alias] = proof
        except Exception as e:
            traceback.print_exc()

    def complete_new_proof(self, text, line, begidx, endidx):
        """
        ...
        """
        # TODO: This just repeats code from load_batch. Refactor into one function
        args = line.split()
        if len(args) == 1:
            arg = ""
        else:
    ...
    def help_new_proof(self):
        print(
            "\n".join(
                [
                    "new_proof alias json_file_with_proof",
                    "Add a new proof to the address book",
                ]
            )
        )
